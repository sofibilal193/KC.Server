...
namespace Kashmir.Captain.Server.Common.Extensions
{
	...
	public abstract class DbRepository<TEntity> : IDbRepository<TEntity> where TEntity : DbContext
	{
		...
		public virtual async Task<TEntity?> FindAsync(params object?[]? keyValues)
		{
			if (!_trackChanges)
			{
				throw new AppDomainUnloadedException("FindAsync method cannot be used with AsNoTracking method.");
			}
			if (_includes.Count > 0 || _stringIncludes.Count > 0)
			{
				throw new AppDomainUnloadedException("FindAsync method cannot be used with Include method.");
			}
